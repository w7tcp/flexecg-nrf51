...
#define DEVICE_NAME                      "FlexECG"                                  /**< Name of device. Will be included in the advertising data. */
...
#define ADC_BUFFER_SIZE									 100						  													/**< size of buffer for incoming ADC data */

static dm_application_instance_t         m_app_handle;                              /**< Application identifier allocated by device manager */

static uint16_t                          m_conn_handle = BLE_CONN_HANDLE_INVALID;   /**< Handle of the current connection. */

static uint16_t										       fifo_buffer[ADC_BUFFER_SIZE];						  /**< buffer for incoming ADC data */

// FROM_SERVICE_TUTORIAL: Declare a service structure for our application
ble_os_t m_our_service;

// 3.G, Declare an app_timer id variable and define our timer interval
APP_TIMER_DEF(m_our_char_timer_id);
#define OUR_CHAR_TIMER_INTERVAL     APP_TIMER_TICKS(40, APP_TIMER_PRESCALER) // 40 ms intervals
                                   
// Declare ADC sample interval and define timer interval
APP_TIMER_DEF(m_our_adc_timer_id);
#define OUR_ADC_TIMER_INTERVAL			APP_TIMER_TICKS(5, APP_TIMER_PRESCALER) // 5 ms intervals
...
// ADC event handler (every 5 ms) - sample input 1 and put data in FIFO buffer
static void adc_timeout_handler(void * p_context)
{
	// do it
}

// Bluetooth characteristic event handler (every 40 ms) - populate packet data from FIFO and send it
static void char_timeout_handler(void * p_context)
{
    // 3.F, Update temperature and characteristic value.
    ...
    {
        // If new data then send notification
        ecg_characteristic_update(&m_our_service, &temperature);
    }
    
    // Save current data until next measurement
...
static void timers_init(void)
{
    ...
    // 3.H, Initiate our timers
    app_timer_create(&m_our_adc_timer_id, APP_TIMER_MODE_REPEATED, adc_timeout_handler);
	  app_timer_create(&m_our_char_timer_id, APP_TIMER_MODE_REPEATED, char_timeout_handler);
}
...
{
    // To update data only when in a connection then don't call app_timer_start() here, but in on_ble_evt().
}
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
        ...
        case BLE_GAP_EVT_CONNECTED:
            ...
            // When connected; start our timers to sample the ADC and send BLE packets
				    app_timer_start(m_our_adc_timer_id, OUR_ADC_TIMER_INTERVAL, NULL);
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            app_timer_stop(m_our_adc_timer_id);
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    // 3.C, Call ble_our_service_on_ble_evt() to do housekeeping of ble connections related to our service and characteristic
...
{   
    ...
    uart_config();
    printf("\033[2J\033[;HCompleted UART init\r\n");
